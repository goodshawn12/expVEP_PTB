function model = train_trca(eeg)

[num_targs, num_chans, num_smpls,~] = size(eeg);
trains = zeros(num_targs, num_chans, num_smpls);
W = zeros(num_targs, num_chans);
for targ_i = 1:1:num_targs
    eeg_tmp = squeeze(eeg(targ_i, :, :, :));
    trains(targ_i,:,:) = squeeze(mean(eeg_tmp, 3));
    w_tmp = trca(eeg_tmp);
    W(targ_i, :) = w_tmp(:,1);
end % targ_i
model = struct('trains', trains, 'W', W, 'num_targs', num_targs);
